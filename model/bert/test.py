...
true_file = './data/bert/test/predicate_out.txt'
...
#print(y_pred[0], mlb.inverse_transform(y_pred[0]))
...
def f1_labels(y_true, y_pred):
    ...
    return f1_list
f1_list = f1_labels(y_true, y_pred)

bad_labels = sorted([(label, score) for label, score in zip(mlb.classes_, f1_list)], key = lambda x: x[1])
print(f"Top 5 labels f1 score: {bad_labels[:10]}")
